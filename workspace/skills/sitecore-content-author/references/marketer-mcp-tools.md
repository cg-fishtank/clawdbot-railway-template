...
| `set_component_datasource` | Set datasource for a component | **In Use** |
| `remove_component_on_page` | Remove component from page | Available |
| `search_site` | Search pages by title/content | **In Use** |
...
| `update_content` | **CRITICAL** — Update fields on content items (image XML, link XML, rich text, etc.) | **In Use** |
...
| `get_content_item_by_id` | Get content item by ID | **In Use** |
...
| `create_component_datasource` | Create datasource item for a component | **In Use** |
...
| `upload_asset` | Upload new digital asset | **In Use** |
...
- Asset Management: 4
- Personalization: 4
- **Author skill uses: 10 tools** (create_page, add_component_on_page, update_content, get_components_on_page, get_page_preview_url, search_site, get_content_item_by_id, create_component_datasource, set_component_datasource, upload_asset)

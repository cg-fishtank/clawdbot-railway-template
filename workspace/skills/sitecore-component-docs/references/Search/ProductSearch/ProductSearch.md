...
The ProductSearch component provides a product search experience using Sitecore Search (RFK). It displays searchable and filterable listings of products with faceted filtering and pagination. The search query is read from URL query parameters, enabling deep linking and bookmarkable search results. This component is typically used on dedicated product catalog or product listing pages.

## Sitecore Template Requirements

### Data Source Template

- **Template Path:** `/sitecore/templates/Project/[Site]/Search/Product Search`
...
|------------|---------------|----------|-------------|------------------------|
| widgetId | Single-Line Text | Yes | Sitecore Search widget ID (RFK ID) for product search | Must match configured Search widget |
...
|---------|-------------|----------|
| Default | `Default` | Standard product search listing (wrapped with `withDatasourceCheck`) |
...
- **What to enter:** The Sitecore Search (Discover) widget ID for product search
- **How to obtain:** Get from Sitecore Search portal under Widgets configuration
- **Format:** Alphanumeric string (e.g., "rfkid_products")
- **Important:** This field MUST be configured for the component to function. Without it, a "No Widget ID" error message displays.

#### PageSizeCount

- **What to enter:** Number of product results to display per page
- **Recommended values:** 12, 16, or 24 (for grid layouts)
- **Example:** `12`

#### facetsToExpand

- **What to enter:** Number of facet groups to show expanded by default
- **Recommended values:** 2-4 for product search (category, brand, price range)
- **Example:** `3`
...
|-----------|-----------------|-----------------|----------|
| Minimal | widgetId | - | Basic product search |
...
|-----------------|-------------|---------|
| searchQuery | Search keyphrase | `?searchQuery=filtration` |

The component uses `router.query.searchQuery` from Next.js router to extract the search term and passes it to the ProductSearchWidget as the `defaultKeyphrase`.
...
- Product search page: `/sitecore/content/[Site]/Home/Products` or `/sitecore/content/[Site]/Home/Catalog`
...
2. Product search widget created in Sitecore Search portal
3. API keys configured in environment variables (SEARCH_CONFIG)
4. All product content types indexed in Sitecore Search
5. `ProductSearchWidget` configured in `src/widgets/SearchResults/ProductSearch/index`

## Page Setup

The product search page should:

1. Be created at a predictable URL (e.g., `/products` or `/catalog`)
2. Have this component as the main content
3. Be linked from the site navigation and product sections
4. Accept search queries via URL query parameters

### Linking to Product Search

From other parts of the site, link to product search using:
```
/products?searchQuery=filtration+system
```
...
2. **Wrong widget type:** Ensure the widget is configured specifically for product content types, not general site search.
...
4. **Wrong content types indexed:** Ensure product pages are indexed in Sitecore Search with appropriate facets (category, brand, specifications).
...
- `PeopleSearch` - Search for people/team members
- `ProductHeader` - Individual product detail page header
...
| Missing products | Widget not configured for products | Verify product content types are indexed |
...
2. Have the ProductSearch rendering ID from the component manifest
3. Know the target placeholder (typically `"headless-main"`)
4. Have a valid Sitecore Search widget ID for product search

### Step 1: Add Component to Page

```javascript
const result = await mcp__marketer-mcp__add_component_on_page({
  pageId: "products-page-guid",
  componentRenderingId: "product-search-rendering-id",
  placeholderPath: "headless-main",
  componentItemName: "ProductSearch_1",
  language: "en",
  fields: {
    "widgetId": "rfkid_products"
  }
});
...
|:------|:-----|:-----------|
| widgetId | Single-Line Text | `"rfkid_products"` |
...
- [ ] widgetId is a valid Sitecore Search widget ID for product search
...
|:------|:------|:---------|
| "Item already exists" | Duplicate component name | Use unique suffix: `ProductSearch_2` |
